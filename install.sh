...
ensure_deps() {
    ...
    if [[ "${PACKAGE_MANAGER}" == "apt" ]] || need_cmd apt-get; then
        ...
        ${SUDO} apt-get install -y -qq curl unzip jq dpkg xvfb xauth flatpak 2>/dev/null || \
        ${SUDO} apt-get install -y -qq curl unzip jq dpkg xvfb xauth
    elif need_cmd dnf; then
        ${SUDO} dnf install -y curl unzip jq cpio rpm xvfb-run xorg-x11-server-Xvfb || \
        ${SUDO} dnf install -y curl unzip jq cpio rpm
    elif need_cmd yum; then
        ${SUDO} yum install -y curl unzip jq cpio rpm
    else
...
show_summary() {
    ...
    echo ""
    log "后台运行 (screen):"
    echo -e "  ${CYAN}screen -dmS napcat bash -c \"xvfb-run -a ${QQ_EXECUTABLE} --no-sandbox\"${NC}"
    echo -e "  ${CYAN}screen -r napcat${NC}"
    echo ""
    log "WebUI Token 查看:"
